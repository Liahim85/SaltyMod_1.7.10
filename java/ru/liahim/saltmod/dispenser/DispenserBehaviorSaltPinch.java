...
public class DispenserBehaviorSaltPinch implements IBehaviorDispenseItem {
	...
	public ItemStack dispense(IBlockSource source, ItemStack stack)
	{
		...
		if (blockFase == ModBlocks.saltDirtLite)
		{
			...
			if (meta == 0 || meta > 2)
			{world.setBlock(x, y, z, ModBlocks.saltDirtLite, 1, 3); stack.splitStack(1); chek = true;}
			else if (meta == 1)
			{world.setBlock(x, y, z, ModBlocks.saltDirtLite, 2, 3); stack.splitStack(1); chek = true;}
			else if (meta == 2)
			{world.setBlock(x, y, z, ModBlocks.saltDirt); stack.splitStack(1); chek = true;}	
		}
		...
		else if (soil == ModBlocks.saltDirtLite && (world.isAirBlock(x, y, z) || blockFase == ModBlocks.saltWort))
		{
			...
			if (meta == 0 || meta > 2)
			{world.setBlock(x, y - 1, z, ModBlocks.saltDirtLite, 1, 3); stack.splitStack(1); chek = true;}
			else if (meta == 1)
			{world.setBlock(x, y - 1, z, ModBlocks.saltDirtLite, 2, 3); stack.splitStack(1); chek = true;}
			else if (meta == 2)
			{world.setBlock(x, y - 1, z, ModBlocks.saltDirt); stack.splitStack(1); chek = true;}
		}
		
		else if (blockFase == ModBlocks.saltDirt) {}
		else if (soil == ModBlocks.saltDirt && (world.isAirBlock(x, y, z) || blockFase == ModBlocks.saltWort)) {}		
		else {BehaviorDefaultDispenseItem.doDispense(world, stack.splitStack(1), 1, enumfacing, iposition); chek = true;}
		
		if (chek) {source.getWorld().playAuxSFX(1000, x, y, z, 0);}
		else {source.getWorld().playAuxSFX(1001, x, y, z, 0);}
