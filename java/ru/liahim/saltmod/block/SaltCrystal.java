...
public class SaltCrystal extends BlockBush {
	...
	{
		if (par1World.doesBlockHaveSolidTopSurface(par1World, x, y-1, z))
		{
		return true;
		}
	...
	protected void crystalFind(World world, int x, int y, int z, EntityPlayer player)
	{
		...
		if (world.provider.dimensionId == 0){if (y<40) player.addStat(AchievSalt.saltCrystal, 1);}
		else if (world.provider.dimensionId == I && CommonProxy.TFOreGen)
	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity)
    {
		if (!world.isRemote){
			    ...
			    EntityList.getEntityString(entity).toLowerCase().contains("witch")))
			{
				...
				world.playSoundEffect((double)x + 0.5D, (double)y + 1.0D, (double)z + 0.5D, "dig.glass", 1.0F, 2.0F);
