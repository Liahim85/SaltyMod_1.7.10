...
public class Extractor extends BlockContainer {
    ...
    private static void playerInvChange(World world, int x, int y, int z, ItemStack held, EntityPlayer player, ItemStack stack)
    {
		...
		if (player.inventory.getCurrentItem() != null)
		{
			...
			{
				world.spawnEntityInWorld(new EntityItem(world, (double)x + 0.5D, (double)y + 1.5D, (double)z + 0.5D, stack));
			}
    ...
    public static void updateExtractorBlockState(boolean burning, boolean extracting, World world, int x, int y, int z)
    {
        ...
        if (burning)
        {
        	if (extracting)	world.setBlock(x, y, z, ModBlocks.extractorSteam);
        	else world.setBlock(x, y, z, ModBlocks.extractorLit);
        }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random)
    {
        if (this.isActive)
        {
            ...
            double f5 = (double)((float)x + random.nextFloat() * 0.4F + 0.3F);
